...
model_file  = OUT_DIR / "lm_transformer_gemma.txt"
weight_file = OUT_DIR / "lm_weight_gemma.txt"
input_file  = OUT_DIR / "lm_input_gemma.txt"
...
def main():
    ...
    print("[INFO] Generating Gemma model...")
    
    model = GemmaForCausalLM(config)
    model.eval()

    print(f"[INFO] Total number of parameters: {model.num_parameters():,}")
    ...
    if apply_quantization is True:
        print(f"[INFO] Fake INT8 Quantization activated.")
    ...
    for i, layer in enumerate(model.model.layers): 
        ...
        # In Gemma, i layer si chiamano gate_proj e up_proj (come LLaMA)

...
def build_config(exp):
    ...
    if exp == 1:
        ...
        # Motivation:
        # Stress on memory bandwidth / multicast routing.
        # 12 query heads and 3 KV heads preserve a 4:1 sharing ratio,
        # similar in spirit to the Qwen GQA experiment.
        # Long sequence length generates enough token traffic to stress the NoC.
        ...
        # Motivation:
        # Stress raw compute throughput with large dense GEMMs.
        # Symmetric attention removes GQA routing asymmetry.
        # A single layer minimizes pipeline effects and isolates MAC utilization.
        # 100,669,440 (~0.101 B) parameters
        ...
        # Motivation:
        # Small matrices reduce arithmetic cost.
        # Multiple sequential layers emphasize synchronization, pipeline bubbles,
        # SRAM traffic, and per-layer overheads rather than raw FLOPS.
        ...
        print("[SETUP] Exp 4: Full-Scale Simulation (~0.29 B, INT8)")
        # Motivation:
        # Realistic larger workload to stress total memory footprint,
        # longer execution paths, and numerical robustness.
        # INT8 fake quantization is enabled to emulate large-scale deployment effects.
        # 290,493,440 (~0.290 B) parameters
        exp_name = "Exp 4: Full-Scale Simulation (~0.29 B, INT8)"
...
def main():
    parser = argparse.ArgumentParser(description="NoCDAS Experiment Generator (Phi-3)")
    parser.add_argument(
        "--exp",
        type=int,
        choices=[1, 2, 3, 4],
    ...
    model_file = OUT_DIR / "phi" / f"lm_transformer_phi_exp{args.exp}.txt"
    weight_file = OUT_DIR / "phi" / f"lm_weight_phi_exp{args.exp}.txt"
    input_file = OUT_DIR / "phi" / f"lm_input_phi_exp{args.exp}.txt"

    os.makedirs(OUT_DIR / "phi", exist_ok=True)

    config, SIMULATION_SEQ_LEN, apply_quantization, exp_name = build_config(args.exp)

    print(f"\n[INFO] Generating Phi-3 architecture for: {exp_name}")
